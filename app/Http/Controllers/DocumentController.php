...
class DocumentController extends Controller
{
    ...
    public function findDataDocs(Request $request, $code_document)
    {
        ...
        // Inisialisasi collections untuk data yang akan diinsert - HARUS SEBELUM DIGUNAKAN
        ...
        if ($riwayatCheck && ($riwayatCheck->status_file == null || $riwayatCheck->status_file == 0)) {
            ...
            $nonQueries = [
                New_product::where('code_document', $code_document)->whereNot('new_status_product', 'sale')->whereNotNull('actual_new_quality->non'),
                StagingProduct::where('code_document', $code_document)->whereNot('new_status_product', 'sale')->whereNotNull('actual_new_quality->non'),
                Product_Bundle::where('code_document', $code_document)->whereNot('new_status_product', 'sale')->whereNotNull('actual_new_quality->non'),
                ProductApprove::where('code_document', $code_document)->whereNot('new_status_product', 'sale')->whereNotNull('actual_new_quality->non'),
                RepairProduct::where('code_document', $code_document)->whereNot('new_status_product', 'sale')->whereNotNull('actual_new_quality->non'),
            ];
            ...
            foreach ($nonQueries as $query) {
                $nonProducts = $nonProducts->merge(
                    $query->select('old_barcode_product', 'new_barcode_product', 'actual_old_price_product')->get()
                );
            }
            ...
            $allBarcodes = collect($damagedProducts)->pluck('new_barcode_product')
                ...
                ->merge(collect($nonProducts)->pluck('new_barcode_product'))
                ->filter()
                ->unique()
                ->values();
            ...
            foreach ($nonProducts as $non) {
                // Skip jika barcode null
                if (empty($non->new_barcode_product)) {
                    continue;
                }

                // Cek apakah sudah ada
                if ($existingProducts->has($non->new_barcode_product)) {
                    // Update old_price_product jika sudah ada
                    $existingProduct = $existingProducts->get($non->new_barcode_product);
                    $existingProduct->update([
                        'old_price_product' => $non->actual_old_price_product,
                    ]);
                } else {
                    // Insert baru jika belum ada
                    ProductDefect::create([
                        'code_document' => $code_document ?? null,
                        'riwayat_check_id' => $riwayatCheck->id,
                        'old_barcode_product' => $non->old_barcode_product,
                        'old_price_product' => $non->actual_old_price_product,
                        'new_barcode_product' => $non->new_barcode_product,
                        'type' => 'non',
                    ]);
                }
            }
